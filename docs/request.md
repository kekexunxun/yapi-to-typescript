...
```typescript
import { RequestFunctionParams } from 'yapi-apifox-to-typescript'
...
- 类型：`'GET' | 'POST' | 'PUT' | 'DELETE' | 'HEAD' | 'OPTIONS' | 'PATCH'`
  ...
  ```typescript
  import { Method } from 'yapi-apifox-to-typescript'
...
- 类型：`'query' | 'form' | 'json' | 'text' | 'file' | 'raw' | 'none'`
  ...
  ```typescript
  import { RequestBodyType } from 'yapi-apifox-to-typescript'
...
- 类型：`'json' | 'text' | 'xml' | 'raw'`
  ...
  ```typescript
  import { ResponseBodyType } from 'yapi-apifox-to-typescript'
